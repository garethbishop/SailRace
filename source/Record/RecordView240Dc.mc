...
class RecordView240Dc
{
    ...
    function PrintCurrentTack(dc, tack)
    {
        dc.setColor(Settings.ForegroundColor, Settings.BackgroundColor);
        ...
        if (tack.equals("port")) {
            tackString = "Port tack";
        } else if (tack.equals("starboard")) {
            tackString = "Starboard tack";
        } else if (tack.equals("tacking")) {
            tackString = "Tacking...";
        } else if (tack.equals("complete")) {
            tackString = "Recording complete";
        }

        var y = 0;
        if (tack.equals("complete")) {
            y = 110;
        } else {
            y = 170;
            // y =  _height / 2 - dc.getFontHeight(Gfx.FONT_MEDIUM) / 2 - dc.getFontHeight(Gfx.FONT_MEDIUM) / 4 - 2;
        }

        dc.drawText(dc.getWidth() / 2, y, Gfx.FONT_MEDIUM, tackString, Gfx.TEXT_JUSTIFY_CENTER);
    ...
    function DisplayState(dc, gpsStatus, recordingStatus)
    {
    	...
    	dc.drawText(105, ty, Gfx.FONT_XTINY, "gps:", Gfx.TEXT_JUSTIFY_RIGHT);
    	dc.drawText(165, ty, Gfx.FONT_XTINY, "rec:", Gfx.TEXT_JUSTIFY_RIGHT);
    	
    	dc.setColor(_gpsColorsArray[gpsStatus], Gfx.COLOR_TRANSPARENT);
        dc.fillCircle(117, ty + dc.getFontHeight(Gfx.FONT_XTINY) / 2 + 2, 8);
        
        dc.setColor(recordingStatus ? Gfx.COLOR_GREEN : Gfx.COLOR_RED, Gfx.COLOR_TRANSPARENT);
        dc.fillCircle(177, ty + dc.getFontHeight(Gfx.FONT_XTINY) / 2 + 2, 8);
    }
